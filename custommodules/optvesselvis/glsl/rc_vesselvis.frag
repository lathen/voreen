...
uniform TransFuncParameters transferFuncVessels_;
uniform TF_SAMPLER_TYPE transferFuncVesselsTex_;
...
uniform VolumeParameters paramStruct_;
uniform sampler3D param_;

uniform float paramAmplification_;
uniform float paramMax_;
...
void rayTraversal(in vec3 first, in vec3 last, float entryDepth, float exitDepth) {
    ...
    WHILE(!finished) {
        ...
        #ifdef PARAM_ENABLED
			float param = getVoxelUnnormalized(param_, paramStruct_, samplePos).a;
			param *= paramAmplification_;
		#endif
		...
		#ifdef PARAM_ENABLED
			float voxel_shift = voxel.a + param;
			vec4 colorVessel = RC_APPLY_CLASSIFICATION(transferFuncVessels_, transferFuncVesselsTex_, voxel_shift, lastIntensity);
            #ifdef COLOR_CODE			
		        vec4 colorParamMin = vec4(1.0, 1.0, 0.0, 1.0);
			    float blendParam = param/paramMax_;
			    blendParam = clamp(blendParam, 0.0, 1.0);
			    //colorParamMin.a = clamp(colorParamMin.a, 0.0, colorVessel.a);
			    //colorVessel = (1.0 - blendParam)*colorVessel + blendParam*colorParamMin;
			    colorVessel.rgb = (1.0 - blendParam)*colorVessel.rgb + blendParam*colorParamMin.rgb;
            #endif			
		#else
			vec4 colorVessel = RC_APPLY_CLASSIFICATION(transferFuncVessels_, transferFuncVesselsTex_, voxel, lastIntensity);
		#endif

		// composite colors
		float blend = colorVessel.a/(color.a + colorVessel.a);
		color.rgb = color.rgb * (1.0 - blend) + colorVessel.rgb * blend;
		color.a = max(color.a, colorVessel.a);
