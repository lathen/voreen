...
void main() {
...
#ifdef PARAM_ENABLED
    float param = getVoxelUnnormalized(param_, paramStruct_, gl_TexCoord[0].xyz).a;
    ...
    if (numChannels_ == 1) {
		...
		// apply vessel classification
		#ifdef PARAM_ENABLED
            float voxel_shift = intensity.a + param;
		    vec4 colorVessel = applyTF(transferFuncVessels_, transferFuncVesselsTex_, voxel_shift);
            #ifdef COLOR_CODE			
            ...
            #endif
		#else
			vec4 colorVessel = applyTF(transferFuncVessels_, transferFuncVesselsTex_, intensity);
		#endif

		// composite colors
		float blend = colorVessel.a/(mat.a + colorVessel.a);
		mat.rgb = mat.rgb * (1.0 - blend) + colorVessel.rgb * blend;
		mat.a = max(mat.a, colorVessel.a);
