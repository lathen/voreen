...
#include "vesselvissliceviewer.h"
...
const std::string VesselVisSliceViewer::loggerCat_("voreen.optvesselvis.VesselVisSliceViewer");
const std::string VesselVisSliceViewer::fontName_("Vera.ttf");

VesselVisSliceViewer::VesselVisSliceViewer()
    : SliceRendererBase()
    ...
    , lastPickingPosition_(-1, -1, -1)
{
    ...
    // interaction
    mouseEventShift_ = new EventProperty<VesselVisSliceViewer>("mouseEvent.Shift", "Slice Shift",
        this, &VesselVisSliceViewer::shiftEvent,
        tgt::MouseEvent::MOUSE_BUTTON_LEFT,
        tgt::MouseEvent::PRESSED | tgt::MouseEvent::MOTION, tgt::Event::CTRL);

    mouseEventMove_ = new EventProperty<VesselVisSliceViewer>("mouseEvent.cursorPositionMove", "Cursor Position Move",
        this, &VesselVisSliceViewer::mouseLocalization,
        tgt::MouseEvent::MOUSE_BUTTON_NONE,
        tgt::MouseEvent::MOTION, tgt::Event::MODIFIER_NONE,
        true);

    mouseEventPress_ = new EventProperty<VesselVisSliceViewer>("mouseEvent.cursorPositionPress", "Cursor Position Press",
        this, &VesselVisSliceViewer::mouseLocalization,
    ...
    sliceAlignment_.onChange(
        CallMemberAction<VesselVisSliceViewer>(this, &VesselVisSliceViewer::onSliceAlignmentChange) );
...
Processor* VesselVisSliceViewer::create() const {
    return new VesselVisSliceViewer();
}

void VesselVisSliceViewer::initialize() throw (tgt::Exception) {
    VolumeRenderer::initialize();

    sliceShader_ = ShdrMgr.loadSeparate("sl_base.vert", "sl_vesselvis.frag", buildShaderHeader(), false);
    LGL_ERROR;
...
void VesselVisSliceViewer::updateSliceProperties() {
...
void VesselVisSliceViewer::process() {
    if (inport_.hasChanged()) {
        ...
        transferFuncVessels_.setVolumeHandle(inport_.getData());
    }
    ...
    if (texMode_.isSelected("2d-texture")) {      // 2D texture
        ...
        transferFunc_.get()->setUniform(sliceShader_, "transferFunc_", "transferFuncTex_", transferUnit.getUnitNumber());
        transferFuncVessels_.get()->setUniform(sliceShader_, "transferFuncVessels_", "transferFuncVesselsTex_", transferUnitVessels.getUnitNumber());
        sliceShader_->setUniform("numChannels_", static_cast<GLint>(inport_.getData()->getNumChannels()));
        LGL_ERROR;
	    ...
	    if (transferFuncVesselsEnableTuned_.get()) {
            ...
            sliceShader_->setUniform("paramAmplification_", transferFuncVessels_.getParamMagnitude());
            if (spatialTFColorCode_.get())
		        sliceShader_->setUniform("paramMax_", paramVol->max());
	    }
        ...
        ++pos, x = pos % numSlicesCol, y = pos / numSlicesCol)
    {
        ...
        if (texMode_.isSelected("2d-texture")) {
            ...
            if (!bindSliceTexture(sliceShader_, inport_.getData(), sliceTexture, &volUnit))
                continue;
...
std::string VesselVisSliceViewer::buildShaderHeader() {
    std::string header = SliceRendererBase::buildShaderHeader();

	if (transferFuncVesselsEnableTuned_.get())
		header += "#define PARAM_ENABLED \n";

	if (spatialTFColorCode_.get())
		header += "#define COLOR_CODE \n";

    return header;
}

void VesselVisSliceViewer::renderSliceBoundaries() const {
...
void VesselVisSliceViewer::renderInfoTexts() const {
...
tgt::vec3 VesselVisSliceViewer::screenToVoxelPos(tgt::ivec2 screenPos) const {
...
tgt::mat4 VesselVisSliceViewer::generatePickingMatrix() const {
...
void VesselVisSliceViewer::onSliceAlignmentChange() {
...
void VesselVisSliceViewer::mouseLocalization(tgt::MouseEvent* e) {
...
void VesselVisSliceViewer::shiftEvent(tgt::MouseEvent* e) {
...
bool VesselVisSliceViewer::singleSliceMode() const {
